...
def generate_random_functions(n, k):
    return [{i: random.randint(0, k - 1) for i in range(n)} for _ in range(n)]
...
def construct_shares_k_out_n(image, k, n, image_label):
    ...
    for i in range(height):
        for j in range(width):
            ...
            for s in range(n):
                mapped_index = H[s][s % k]
                shares[s, i, j * num_subpixels: (j + 1) * num_subpixels] = permuted_pattern[mapped_index]
...
def share_construction():
    ...
    k = simpledialog.askinteger("Input", "Enter k (minimum shares required for reconstruction):")
    n = simpledialog.askinteger("Input", "Enter n (total number of shares):")
    if not k or not n:
        return
...
def share_reconstruction():
    k = simpledialog.askinteger("Input", "Enter the number of shares to use for reconstruction:")
    if not k:
        return
    selected_shares = []
    for i in range(k):
...
def main():
    root = tk.Tk()
    root.title("k-out-of-n Secret Sharing Scheme")
